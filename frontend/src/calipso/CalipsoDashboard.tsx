...
const GoalsPage = () => {
  ...
  const iconMapping: Record<string, any> = {
    Target, Car, Building2, Plane, GraduationCap, HeartPulse
  };
  ...
  const formattedGoals = goals.map((g, i) => {
    ...
    return {
      ...
      iconCmp: <IconCmp className="w-5 h-5" />
    };
  });

  const chartData = formattedGoals.length > 0 ? formattedGoals : [{ name: 'Empty', progress: 100, color: '#f3f4f6', id: 'empty', label: 'Empty' }];
               ...
               {formattedGoals.map((goal) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity absolute right-4 bg-white/90 p-2 rounded-xl backdrop-blur-sm shadow-xl border border-gray-50">
                      <button 
                        onClick={(e) => { e.stopPropagation(); setSelectedGoal(goal); setIsAdjustModalOpen(true); }}
                        className="w-8 h-8 rounded-full bg-lime-100 text-lime-700 flex items-center justify-center hover:bg-lime-200"
                        title="Add Savings"
                      >
           ...
           <div className="bg-gray-900 rounded-[48px] p-10 text-white relative overflow-hidden h-fit">
              <div className="relative z-10">
                ...
                <div className="flex gap-4">
                   ...
                   >
                     {goals.length > 0 ? "Adjust Top Goal" : "Create Goal"}
                   </button>
