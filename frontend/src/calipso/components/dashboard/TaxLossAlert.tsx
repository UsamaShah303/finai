...
export const TaxLossAlert: React.FC<TaxLossAlertProps> = ({ holding, onHarvest, onDismiss }) => {
  ...
  const handleHarvest = async () => {
    ...
    const bookedLoss = holding.pkr_loss || Math.max(0, (holding.avg_buy_price - holding.current_price) * holding.shares);
    const taxSaved = holding.pkr_tax_saved || Math.round(bookedLoss * 0.15);
    setResult({
      sold: holding.symbol,
      bought: REPLACEMENTS[holding.symbol] || 'Sector ETF',
      loss_harvested: bookedLoss,
      pkr_loss: bookedLoss,
      tax_saved: taxSaved,
      pkr_tax_saved: taxSaved,
      new_holding: REPLACEMENTS[holding.symbol] || 'Sector ETF',
    });
    setHarvested(true);
    setLoading(false);
    setTimeout(() => {
