...
const ASSETS = ['VTI', 'BND', 'OGDC', 'ENGRO', 'LUCK'];

const buildMockShapData = (symbol: string): ShapData => ({
  symbol,
  confidence: symbol === 'ENGRO' ? 78 : 86,
  summary: `${symbol} fits the current model because it improves diversification while keeping the portfolio aligned with your moderate growth target.`,
  summary_ur: 'یہ اثاثہ آپ کے پورٹ فولیو میں تنوع اور متوازن ترقی کے ہدف کو بہتر بناتا ہے۔',
  factors: [
    {
      key: 'diversification',
      title: 'Diversification Lift',
      desc: 'The asset reduces concentration risk and balances local and global market exposure.',
      ur: 'یہ اثاثہ ارتکاز کے خطرے کو کم کرتا ہے اور مقامی و عالمی مارکیٹ ایکسپوژر کو متوازن بناتا ہے۔',
      value: 0.42,
      direction: 'positive',
      icon: '↗',
    },
    {
      ...
      desc: 'Recent price and earnings signals are stronger than the basket average.',
      ur: 'حالیہ قیمت اور آمدنی کے اشارے اوسط سے بہتر ہیں۔',
      value: 0.31,
      direction: 'positive',
      icon: '★',
    },
    {
      ...
      desc: 'The model is keeping position size controlled because valuation is no longer deeply discounted.',
      ur: 'قیمت مناسب حد سے اوپر ہے، اس لیے ماڈل پوزیشن کو محدود رکھتا ہے۔',
      value: 0.22,
      direction: 'concern',
      icon: '!',
    },
  ...
  what_if: [
    {
      scenario: 'If volatility rises by 10%',
      impact: 'The allocation would shift 4% toward bonds and cash while keeping the core holding active.',
    },
    {
      scenario: 'If expected return drops',
      impact: 'The system would rebalance into lower-cost broad-market exposure.',
    },
  ],
});
...
export const SHAPExplainer = () => {
  ...
  useEffect(() => {
    const fetchShapData = async () => {
      ...
      setData(buildMockShapData(selectedAsset));
      setLoading(false);
    };
        ...
        <div className="lg:col-span-12 xl:col-span-7 space-y-6">
          <div className="bg-white/40 backdrop-blur-sm rounded-[56px] p-10 lg:p-14 border border-white/60 shadow-sm relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              {data.factors.filter(f => f.direction === 'positive').map((f, i) => (
              ...
              >
                ...
                >
                  <div className="flex items-center gap-5">
                    ...
                    <span className="text-base font-black text-gray-800 tracking-tight leading-tight">{item.scenario}</span>
                  </div>
                    ...
                    >
                      <div className="px-10 pb-10 pt-2">
                        <motion.div 
                           ...
                           className="p-8 bg-white rounded-[32px] border border-indigo-100/50 shadow-sm flex gap-5"
                        >
