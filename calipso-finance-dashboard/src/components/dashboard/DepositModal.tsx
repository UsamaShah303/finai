...
export const DepositModal: React.FC<DepositModalProps> = ({ isOpen, onClose, onSuccess, riskScore }) => {
  ...
  const handleDeposit = async () => {
    ...
    try {
      // 1. Credit wallet
      const depositRes = await authApi.depositFunds(parseFloat(amount), currency);
      
      // 2. Transition to investing animation
      setStep('INVESTING');
      
      // 3. Simulate AI pipeline delay
      await new Promise(resolve => setTimeout(resolve, 4000));
      
      // 4. Auto invest
      await authApi.autoInvest(parseFloat(amount), currency, riskScore);
      
      // 5. Update local state
      updateUser({ wallet: depositRes.wallet });
          ...
          <div className="p-8 pb-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
               ...
               <div>
                  ...
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Secure AI Bridge</p>
               </div>
            </div>
            <button 
              onClick={onClose}
                    ...
                    <div className="flex justify-center">
                      <div className="inline-flex bg-gray-100 p-1 rounded-2xl">
                        <button 
                          ...
                          className={`px-6 py-2 rounded-xl text-xs font-black transition-all ${currency === 'USD' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-400'}`}
                        >USD</button>
                        <button 
                          onClick={() => setCurrency('PKR')}
                          className={`px-6 py-2 rounded-xl text-xs font-black transition-all ${currency === 'PKR' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-400'}`}
                        >PKR</button>
                      </div>
                    </div>

                    <div className="relative group">
                       <span className="absolute left-8 top-1/2 -translate-y-1/2 text-2xl font-black text-gray-400 group-focus-within:text-primary transition-colors">
                          {currency === 'USD' ? '$' : '₨'}
                       </span>
                       <input 
                         type="number"
                ...
                >
                  ...
                  <div className="bg-gray-50 rounded-[40px] p-8 space-y-6">
                     <div className="flex justify-between items-center pb-6 border-b border-gray-200/60">
                        <span className="text-sm font-bold text-gray-400 uppercase tracking-widest">Amount</span>
                        <span className="text-3xl font-black text-gray-900">
                          {currency === 'USD' ? '$' : '₨'}{parseFloat(amount).toLocaleString()}
                        </span>
                     </div>
                     <div className="flex justify-between items-center pb-6 border-b border-gray-200/60">
                        <span className="text-sm font-bold text-gray-400 uppercase tracking-widest">Strategy</span>
                     ...
                     <div className="flex justify-between items-center">
                        <span className="text-sm font-bold text-gray-400 uppercase tracking-widest">Target</span>
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <button
                      ...
                      className="w-full text-gray-400 py-4 font-black uppercase tracking-widest text-[10px] hover:text-gray-900 transition-all"
                    >
                      Back to edit
                    </button>
                  </div>
